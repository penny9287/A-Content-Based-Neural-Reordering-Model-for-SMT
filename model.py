...
def _predefined_model(args, embedding_matrix):
    ...
    (filtersize_list, number_of_filters_per_filtersize, pool_length_list,
     dropout_list, optimizer, use_embeddings, embeddings_trainable) = _param_selector(args)

    if (use_embeddings):
        embedding_layer = Embedding(args.nb_words + 1, args.embedding_dim, weights=[embedding_matrix],
                                    input_length=args.max_sequence_len, trainable=embeddings_trainable)
    else:
        embedding_layer = Embedding(args.nb_words + 1, args.embedding_dim, weights=None, input_length=args.max_sequence_len,
...
def _param_selector(args):
    ...
    number_of_filters_per_filtersize = [10, 10, 10]
    ...
    if (args.model_name.lower() == 'cnn-rand'):
        ...
        filtersize_list = [3, 4, 5]
        number_of_filters_per_filtersize = [15, 15, 15]
        pool_length_list = [2, 2, 2]
        dropout_list = [0.25, 0.5]
        optimizer = RMSprop(lr=args.learning_rate, decay=args.decay_rate, clipvalue=args.grad_clip)
        use_embeddings = True
        embeddings_trainable = True
